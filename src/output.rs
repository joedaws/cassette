// The flat-note writer lives here until Phase 6 deletes it. Phase 5a moved
// the TUI onto the session store, so nothing in the non-test build calls
// `write_markdown`, `parse_markdown`, `parse_append_base` or their helpers
// any more — they are kept, not dead by accident. `cassette_body`, the
// store-shaped body builder, is the one item here with a live caller.
// Remove this when Phase 6 deletes the flat-note format.
#![allow(dead_code)]
